jQuery(document).ready(function ($) {
	...
	function insert_rule(inputs) {
		...
		var $keep_query_input = $('<input type="hidden" name="keep_query[]" class="keep-query-input" value="' + (keep_query ? '1' : '0') + '">');
		...
		var $keep_query_checkbox = $('<label>').html('<input type="checkbox" class="keep-query-checkbox"' + ((keep_query) ? ' checked' : '') + '> Keep query string.');
		$keep_query_checkbox.find('input[type="checkbox"]').on('change', function () {
			$(this).parent().parent().find('.keep-query-input').val(($(this).is(':checked')) ? 1 : 0);
		});
		...
		var $from_list = $('<select name="from[]" data-placeholder="Choose Page..." class="chosen redirect-from">')
			...
			.on('change', function () {
				...
				if ($(this).val() == 'url') {
					...
					$(this).parent().find('.post-container .chosen-search input').autocomplete({
		...
		if (from.substr(0, 4) == 'post' || from.substr(0, 4) == 'page') {
			var parts = from.split(';');

			items.from.id = parts[0];
			items.from.name = parts[1];

			$from_list.find('option[value="post"]').attr('selected', '');
		}

		if (to.substr(0, 4) == 'post' || to.substr(0, 4) == 'page') {
			var parts = to.split(';');

			items.to.id = parts[0];
			items.to.name = parts[1];

			$destination_list.find('option[value="post"]').attr('selected', '');
		}
		...
		$post_from_list.parent().find('.chosen-search input').autocomplete({
			source: function(request, response) {
				$.post(ajaxurl, {
					action: 'ip2location_redirection_search_post',
					search: request.term,
					...
					response($.map(data, function(item) {
						$post_from_list.append('<option value="' + item.id + '">' + item.name + '</option>');
					}));

					$post_from_list.trigger('chosen:updated');
				}).always(function () {});
			}
		});

		$post_to_list.parent().find('.chosen-search input').autocomplete({
			source: function(request, response) {
				$.post(ajaxurl, {
					action: 'ip2location_redirection_search_post',
					search: request.term,
					...
					response($.map(data, function(item) {
						$post_to_list.append('<option value="' + item.id + '">' + item.name + '</option>');
					}));

					$post_to_list.trigger('chosen:updated');
				}).always(function () {});
			}
		});

		if (items.from.id) {
			$post_from_list.append('<option value="' + items.from.id + '" selected> ' + items.from.name + '</option>').trigger('chosen:updated');
		}

		if (items.to.id) {
			$post_to_list.append('<option value="' + items.to.id + '" selected> ' + items.to.name + '</option>').trigger('chosen:updated');
		}

		if ($post_from_list.is(':empty')) {
			$post_from_list.append('<option value="" selected> Search...</option>').trigger('chosen:updated');
		}

		if ($post_to_list.is(':empty')) {
			$post_to_list.append('<option value="" selected> Search...</option>').trigger('chosen:updated');
		}
	...
	$('#btn-get-started').on('click', function (e) {
		...
		$('#modal-step-1').css('display', 'block'); //go to query selection page
	});

	//if query selection is made
	$('#btn-to-step-1').on('click', function (e) {
		e.preventDefault();
		var sel = $('input[name="ipl-sel"]:checked').val();
	...
	$('#btn-to-api-step-2').on('click', function (e) {
		...
		$.post(
			ajaxurl,
			{
				action: 'ip2location_redirection_validate_api_key',
