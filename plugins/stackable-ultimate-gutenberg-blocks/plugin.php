...
if ( defined( 'STACKABLE_FILE' ) && STACKABLE_FILE !== __FILE__ && !isset( $GLOBALS['OTHER_STACKABLE_FILE'] ) && defined( 'STACKABLE_BUILD' ) && defined( 'STACKABLE_VERSION' ) ) {
    ...
    add_option( 'stackable_other_stackable_plugin_info', [
        'BUILD'   => STACKABLE_BUILD,
...
defined( 'STACKABLE_VERSION' ) || define( 'STACKABLE_VERSION', '3.19.5' );
defined( 'STACKABLE_FILE' ) || define( 'STACKABLE_FILE', __FILE__ );
defined( 'STACKABLE_I18N' ) || define( 'STACKABLE_I18N', 'stackable-ultimate-gutenberg-blocks' );
// Plugin slug.
defined( 'STACKABLE_DESIGN_LIBRARY_URL' ) || define( 'STACKABLE_DESIGN_LIBRARY_URL', 'https://stackable-files.pages.dev' );
// Design Library CDN URL
/********************************************************************************************
...
if ( !function_exists( 'stackable_notice_other_stackable_plugin_deactivated' ) ) {
    function stackable_notice_other_stackable_plugin_deactivated() {
        ...
        if ( $OTHER_STACKABLE_INFO ) {
            printf( '<div class="notice notice-info is-dismissible stackable_notice_gutenberg_plugin"><p>%s</p></div>', sprintf(
                esc_html__( '%sStackable Notice%s: The Stackable plugin (%s version %s) has been deactivated. Only one active Stackable plugin is needed.', STACKABLE_I18N ),
                '<strong>',
                '</strong>',
                $OTHER_STACKABLE_INFO['BUILD'],
                $OTHER_STACKABLE_INFO['VERSION']
            ) );
...
if ( is_admin() ) {
    require_once plugin_dir_path( __FILE__ ) . 'src/welcome/useful-plugins.php';
    // For cross-marketing
